<template>
  ...
  ]" class="relative select-none transition-300 active:scale-95 leading-6" v-bind="{ type, disabled }">
    <span :class="[
      {
        'opacity-0!': loading,
      ...
      <slot name="icon" />
      <span v-if="text?.length" :class="[variantTexts[variant], { 'text-gray-300!': disabled }]"
        class="whitespace-nowrap">
        {{ text }}
      </span>
      <span v-if="icon?.length" :class="[icon, { 'text-base': size === 'md' }, variantTexts[variant]]"></span>
    </span>
    <transition mode="out-in" name="fade">
      <span v-if="loading" class="absolute-center z-10 inline-block! w-max h-max loading transition-300">
        <svg class="animate-spin" fill="none" height="20" viewBox="0 0 20 20" width="20"
          xmlns="http://www.w3.org/2000/svg">
          <path :fill="loaderFill"
...
const variantTexts: Record<TButtonVariants, string> = {
  ...
  gray: "text-black font-semibold text-sm",
};
